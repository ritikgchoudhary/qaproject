...
<body class="bg-[#050505] text-white min-h-screen flex">
    ...
    <!-- Agent Dashboard Modal (Hidden) -->
    <div id="agentDashboardModal" class="hidden fixed inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-[#121212] border border-white/10 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto p-6 relative shadow-2xl">
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-2xl font-bold text-white">Agent Dashboard</h3>
                <button onclick="document.getElementById('agentDashboardModal').classList.add('hidden')" class="text-gray-400 hover:text-white">
    ...
    <!-- Direct Commission Modal (Hidden) -->
    <div id="commissionModal" class="hidden fixed inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-[#121212] border border-white/10 rounded-2xl w-full max-w-md p-6 relative shadow-2xl">
            <h3 class="text-xl font-bold mb-4 text-white">Set Direct Commission Percentage</h3>
            <p class="text-sm text-gray-500 mb-6">Agent: <span id="commission_agent_name" class="text-white font-bold"></span></p>
            <p class="text-xs text-gray-400 mb-4">This percentage will be applied to direct user's first deposit. Default is 50%.</p>
            
            <form id="commissionForm" class="space-y-4">
                <input type="hidden" id="commission_agent_id">
                
                <div>
                     <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Commission Percentage (%)</label>
                     <input type="number" step="0.01" min="0" max="100" id="commission_percentage" class="w-full bg-[#1a1a1a] border border-white/10 rounded-lg px-3 py-2 text-white text-sm" placeholder="50.00" required>
                     <p class="text-xs text-gray-500 mt-1">Enter a value between 0 and 100</p>
                </div>

                <div class="flex gap-2 mt-4">
                    <button type="button" onclick="document.getElementById('commissionModal').classList.add('hidden')" class="flex-1 bg-gray-800 hover:bg-gray-700 text-white py-2 rounded-lg text-sm font-bold">Cancel</button>
                    <button type="submit" class="flex-1 bg-yellow-500 hover:bg-yellow-400 text-black py-2 rounded-lg text-sm font-bold">Update Commission</button>
                </div>
            </form>
        </div>
    </div>
    ...
    // Direct Commission Logic
    function editDirectCommission(agentId, agentName, currentPercentage) {
        document.getElementById('commission_agent_id').value = agentId;
        document.getElementById('commission_agent_name').innerText = agentName;
        document.getElementById('commission_percentage').value = currentPercentage || 50;
    ...
    document.getElementById('commissionForm').addEventListener('submit', function(e) {
        e.preventDefault();
        const data = {
            action: 'update_direct_commission',
            agent_id: document.getElementById('commission_agent_id').value,
            percentage: parseFloat(document.getElementById('commission_percentage').value)
        };
        ...
        .then(data => {
            if(data.success) {
                alert('Direct commission percentage updated successfully!');
                document.getElementById('commissionModal').classList.add('hidden');
                location.reload();
            } else {
                alert('Error: ' + (data.error || 'Failed to update commission percentage'));
            }
        })
        .catch(err => {
                        ...
                        tr.innerHTML = `
                            ...
                            <td class="p-4 text-right">
                                <div class="flex justify-end gap-2 text-right">
                                    ${u.role === 'agent' ? `
                                    <button onclick="editDirectCommission(${u.id}, '${u.name.replace(/'/g, "\\'")}', ${u.direct_commission_percentage || 50})" class="bg-indigo-500/10 hover:bg-indigo-500 hover:text-white text-indigo-500 p-2 rounded-lg transition-all" title="Set Direct Commission (${u.direct_commission_percentage || 50}%)">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
                                    </button>
                                    <button onclick="viewAgentDashboard(${u.id}, '${u.name.replace(/'/g, "\\'")}')" class="bg-orange-500/10 hover:bg-orange-500 hover:text-white text-orange-500 p-2 rounded-lg transition-all" title="Agent Dashboard">
    ...
    function viewAgentDashboard(agentId, agentName) {
        document.getElementById('agentDashboardModal').classList.remove('hidden');
        const content = document.getElementById('agentDashboardContent');
        content.innerHTML = `
            <div class="text-center">
                <div class="inline-block w-8 h-8 border-2 border-yellow-500 border-t-transparent rounded-full animate-spin"></div>
                <p class="text-gray-400 mt-2">Loading agent statistics...</p>
        ...
        fetch(`api_action.php?action=get_agent_stats&agent_id=${agentId}`)
            .then(res => res.json())
            .then(data => {
                if (data.error) {
                    content.innerHTML = `<div class="text-red-400 text-center">Error: ${data.error}</div>`;
                    return;
                }
                ...
                content.innerHTML = `
                    <div class="bg-[#1a1a1a] rounded-xl p-4 mb-4">
                        <h4 class="text-lg font-bold text-white mb-2">${stats.agent_info.name}</h4>
                        <p class="text-sm text-gray-400">Referral Code: <span class="text-yellow-400 font-mono">${stats.agent_info.referral_code}</span></p>
                    </div>

                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <!-- Users Joined -->
                        <div class="bg-[#1a1a1a] border border-white/5 rounded-xl p-4">
                            <div class="text-xs text-gray-500 uppercase mb-2">Users Joined</div>
                            <div class="space-y-1">
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-400">Today:</span>
                                    <span class="text-white font-bold">${stats.users_joined.today}</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-400">Yesterday:</span>
                                    <span class="text-white font-bold">${stats.users_joined.yesterday}</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-400">This Month:</span>
                                    <span class="text-white font-bold">${stats.users_joined.this_month}</span>
                                </div>
                                <div class="flex justify-between text-sm border-t border-white/10 pt-1 mt-1">
                                    <span class="text-gray-400">All:</span>
                                    <span class="text-yellow-400 font-bold">${stats.users_joined.all}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Deposits -->
                        <div class="bg-[#1a1a1a] border border-white/5 rounded-xl p-4">
                            <div class="text-xs text-gray-500 uppercase mb-2">Deposits</div>
                            <div class="space-y-1">
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-400">Today:</span>
                                    <span class="text-white font-bold">₹${Number(stats.deposits.today).toLocaleString('en-IN', {minimumFractionDigits: 2})}</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-400">Yesterday:</span>
                                    <span class="text-white font-bold">₹${Number(stats.deposits.yesterday).toLocaleString('en-IN', {minimumFractionDigits: 2})}</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-400">This Month:</span>
                                    <span class="text-white font-bold">₹${Number(stats.deposits.this_month).toLocaleString('en-IN', {minimumFractionDigits: 2})}</span>
                                </div>
                                <div class="flex justify-between text-sm border-t border-white/10 pt-1 mt-1">
                                    <span class="text-gray-400">All:</span>
                                    <span class="text-yellow-400 font-bold">₹${Number(stats.deposits.all).toLocaleString('en-IN', {minimumFractionDigits: 2})}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Commissions -->
                        <div class="bg-[#1a1a1a] border border-white/5 rounded-xl p-4">
                            <div class="text-xs text-gray-500 uppercase mb-2">Commissions</div>
                            <div class="space-y-1">
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-400">Today:</span>
                                    <span class="text-green-400 font-bold">₹${Number(stats.commissions.today).toLocaleString('en-IN', {minimumFractionDigits: 2})}</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-400">Yesterday:</span>
                                    <span class="text-green-400 font-bold">₹${Number(stats.commissions.yesterday).toLocaleString('en-IN', {minimumFractionDigits: 2})}</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-400">This Month:</span>
                                    <span class="text-green-400 font-bold">₹${Number(stats.commissions.this_month).toLocaleString('en-IN', {minimumFractionDigits: 2})}</span>
                                </div>
                                <div class="flex justify-between text-sm border-t border-white/10 pt-1 mt-1">
                                    <span class="text-gray-400">All:</span>
                                    <span class="text-yellow-400 font-bold">₹${Number(stats.commissions.all).toLocaleString('en-IN', {minimumFractionDigits: 2})}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Wallet -->
                        <div class="bg-[#1a1a1a] border border-white/5 rounded-xl p-4">
                            <div class="text-xs text-gray-500 uppercase mb-2">Wallet</div>
                            <div class="space-y-2">
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-400">Withdrawable:</span>
                                    <span class="text-green-400 font-bold">₹${Number(stats.wallet.withdrawable).toLocaleString('en-IN', {minimumFractionDigits: 2})}</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-400">Total Earned:</span>
                                    <span class="text-yellow-400 font-bold">₹${Number(stats.wallet.total_earned).toLocaleString('en-IN', {minimumFractionDigits: 2})}</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-400">Total Withdrawn:</span>
                                    <span class="text-gray-400 font-bold">₹${Number(stats.wallet.total_withdrawn).toLocaleString('en-IN', {minimumFractionDigits: 2})}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            .catch(err => {
                content.innerHTML = `<div class="text-red-400 text-center">Error loading agent statistics: ${err.message}</div>`;
            });
    }
    </script>
</body>
