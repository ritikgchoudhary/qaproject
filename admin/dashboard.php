...
// Deposit Statistics: Today, Yesterday, This Month
...
$stmt = $pdo->prepare("SELECT SUM(amount) FROM deposits WHERE status IN ('success', 'approved', 'completed') AND created_at >= ? AND created_at <= ?");
$stmt->execute([$today_start, $today_end]);
$deposits_today = $stmt->fetchColumn() ?: 0;

$stmt = $pdo->prepare("SELECT SUM(amount) FROM deposits WHERE status IN ('success', 'approved', 'completed') AND created_at >= ? AND created_at <= ?");
$stmt->execute([$yesterday_start, $yesterday_end]);
$deposits_yesterday = $stmt->fetchColumn() ?: 0;

$stmt = $pdo->prepare("SELECT SUM(amount) FROM deposits WHERE status IN ('success', 'approved', 'completed') AND created_at >= ? AND created_at <= ?");
$stmt->execute([$month_start, $month_end]);
$deposits_this_month = $stmt->fetchColumn() ?: 0;

// User Statistics: Today, Yesterday, This Month, All
$stmt = $pdo->prepare("SELECT COUNT(*) FROM users WHERE created_at >= ? AND created_at <= ?");
$stmt->execute([$today_start, $today_end]);
$users_today = $stmt->fetchColumn() ?: 0;

$stmt = $pdo->prepare("SELECT COUNT(*) FROM users WHERE created_at >= ? AND created_at <= ?");
$stmt->execute([$yesterday_start, $yesterday_end]);
$users_yesterday = $stmt->fetchColumn() ?: 0;

$stmt = $pdo->prepare("SELECT COUNT(*) FROM users WHERE created_at >= ? AND created_at <= ?");
$stmt->execute([$month_start, $month_end]);
$users_this_month = $stmt->fetchColumn() ?: 0;
...
<body class="bg-[#050505] text-white min-h-screen flex">
    ...
    <main class="ml-64 flex-1 p-8">
        ...
        <!-- Deposit Statistics: Today, Yesterday, This Month -->
        <h2 class="text-xl font-bold mb-6 text-gray-400">Deposit Statistics</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            ...
            <div class="bg-[#111] border border-white/5 p-6 rounded-2xl relative overflow-hidden group hover:border-white/10 transition-all">
                ...
                <p class="text-gray-500 text-xs font-bold uppercase tracking-widest mb-1">Today's Deposits</p>
                <p class="text-3xl font-black text-blue-400">₹<?= number_format($deposits_today, 2) ?></p>
                <p class="text-[10px] text-gray-500 mt-2"><?= date('d M Y') ?></p>
            </div>

            <!-- Yesterday's Deposits -->
            <div class="bg-[#111] border border-white/5 p-6 rounded-2xl relative overflow-hidden group hover:border-white/10 transition-all">
                <div class="absolute top-0 right-0 w-24 h-24 bg-purple-500/10 rounded-full blur-2xl -mr-4 -mt-4 group-hover:bg-purple-500/20 transition-all"></div>
                <p class="text-gray-500 text-xs font-bold uppercase tracking-widest mb-1">Yesterday's Deposits</p>
                <p class="text-3xl font-black text-purple-400">₹<?= number_format($deposits_yesterday, 2) ?></p>
                <p class="text-[10px] text-gray-500 mt-2"><?= date('d M Y', strtotime('-1 day')) ?></p>
            </div>

            <!-- This Month's Deposits -->
            <div class="bg-[#111] border border-white/5 p-6 rounded-2xl relative overflow-hidden group hover:border-white/10 transition-all">
                <div class="absolute top-0 right-0 w-24 h-24 bg-cyan-500/10 rounded-full blur-2xl -mr-4 -mt-4 group-hover:bg-cyan-500/20 transition-all"></div>
                <p class="text-gray-500 text-xs font-bold uppercase tracking-widest mb-1">This Month's Deposits</p>
                <p class="text-3xl font-black text-cyan-400">₹<?= number_format($deposits_this_month, 2) ?></p>
                <p class="text-[10px] text-gray-500 mt-2"><?= date('M Y') ?></p>
            </div>
        </div>

        <!-- User Statistics: Today, Yesterday, This Month, All -->
        <h2 class="text-xl font-bold mb-6 text-gray-400">User Statistics</h2>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
            <!-- Today's New Users -->
            <div class="bg-[#111] border border-white/5 p-6 rounded-2xl relative overflow-hidden group hover:border-white/10 transition-all">
                <div class="absolute top-0 right-0 w-24 h-24 bg-emerald-500/10 rounded-full blur-2xl -mr-4 -mt-4 group-hover:bg-emerald-500/20 transition-all"></div>
                <p class="text-gray-500 text-xs font-bold uppercase tracking-widest mb-1">Today Joined</p>
                <p class="text-3xl font-black text-emerald-400"><?= number_format($users_today) ?></p>
                <p class="text-[10px] text-gray-500 mt-2"><?= date('d M Y') ?></p>
            </div>

            <!-- Yesterday's New Users -->
            <div class="bg-[#111] border border-white/5 p-6 rounded-2xl relative overflow-hidden group hover:border-white/10 transition-all">
                <div class="absolute top-0 right-0 w-24 h-24 bg-pink-500/10 rounded-full blur-2xl -mr-4 -mt-4 group-hover:bg-pink-500/20 transition-all"></div>
                <p class="text-gray-500 text-xs font-bold uppercase tracking-widest mb-1">Yesterday Joined</p>
                <p class="text-3xl font-black text-pink-400"><?= number_format($users_yesterday) ?></p>
                <p class="text-[10px] text-gray-500 mt-2"><?= date('d M Y', strtotime('-1 day')) ?></p>
            </div>

            <!-- This Month's New Users -->
            <div class="bg-[#111] border border-white/5 p-6 rounded-2xl relative overflow-hidden group hover:border-white/10 transition-all">
                <div class="absolute top-0 right-0 w-24 h-24 bg-indigo-500/10 rounded-full blur-2xl -mr-4 -mt-4 group-hover:bg-indigo-500/20 transition-all"></div>
                <p class="text-gray-500 text-xs font-bold uppercase tracking-widest mb-1">This Month Joined</p>
                <p class="text-3xl font-black text-indigo-400"><?= number_format($users_this_month) ?></p>
                <p class="text-[10px] text-gray-500 mt-2"><?= date('M Y') ?></p>
            </div>

            <!-- Total Users -->
            <div class="bg-[#111] border border-white/5 p-6 rounded-2xl relative overflow-hidden group hover:border-white/10 transition-all">
                <div class="absolute top-0 right-0 w-24 h-24 bg-white/10 rounded-full blur-2xl -mr-4 -mt-4 group-hover:bg-white/20 transition-all"></div>
                ...
                <p class="text-[10px] text-gray-500 mt-2">All Time</p>
            </div>
        </div>
