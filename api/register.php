...
if ($ref_code_input) {
    $stmt = $pdo->prepare("SELECT id, referral_code, role FROM users WHERE referral_code = ?");
    ...
    if ($referrer_data) {
        ...
        $referrer_role = $referrer_data['role'] ?? 'user';

        // Check referral limit ONLY for regular users (not for agents)
        // Agents have UNLIMITED referrals
        ...
        // If referrer is an agent, skip the limit check (unlimited referrals)
    }
}
