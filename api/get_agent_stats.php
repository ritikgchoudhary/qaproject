...
function getCommissionFromUser($pdo, $agent_id, $from_uid) {
    $stmt = $pdo->prepare("SELECT SUM(COALESCE(adjusted_amount, amount)) FROM agent_commissions WHERE agent_id = ? AND from_user_id = ? AND status = 'approved'");
    $stmt->execute([$agent_id, $from_uid]);
    return $stmt->fetchColumn() ?: 0;
}

// 1. Total Global Earnings (only approved commissions)
$stmt = $pdo->prepare("SELECT SUM(COALESCE(adjusted_amount, amount)) FROM agent_commissions WHERE agent_id = ? AND status = 'approved'");
$stmt->execute([$user_id]);
$total_earnings = $stmt->fetchColumn() ?: 0;

// 2. Recent Commissions (only approved)
$stmt = $pdo->prepare("
    SELECT ac.*, u.name as from_user_name, u.email as from_user_email 
    FROM agent_commissions ac 
    JOIN users u ON ac.from_user_id = u.id 
    WHERE ac.agent_id = ? AND ac.status = 'approved'
