...
async function requestWithdraw() {
    ...
    const totalBalance = parseFloat(userStore.wallet?.withdrawable_balance || 0)
    if (parseFloat(amount.value) > totalBalance) {
        error.value = "अपर्याप्त बैलेंस (Insufficient Balance)"
        return
    }
    // Check max withdrawal limit based on level
    if (reqData.value.max_withdraw_amount && parseFloat(amount.value) > reqData.value.max_withdraw_amount) {
        error.value = `Maximum withdrawal for Level ${reqData.value.current_level} is ₹${reqData.value.max_withdraw_amount}`
        return
    }
...
<template>
  <div class="withdraw-page">
      ...
      <!-- Max Withdraw Info -->
      <div class="glass-card info-card" v-if="reqData.max_withdraw_amount">
          <div class="info-row">
              <div class="info-icon">💰</div>
              <div class="info-content">
                  <span class="info-label">Maximum Withdrawal</span>
                  <span class="info-value text-gold">₹{{ reqData.max_withdraw_amount }}</span>
              </div>
          </div>
          <p class="info-desc" v-if="reqData.current_level == 1">
              You can withdraw up to ₹{{ reqData.max_withdraw_amount }} at Level {{ reqData.current_level }}
          </p>
          <p class="info-desc" v-else>
              You can withdraw up to ₹{{ reqData.max_withdraw_amount }} (Level {{ reqData.current_level - 1 }} amount)
          </p>
      </div>

      <!-- Unlock Requirements (Hidden - Locked balance system removed) -->
      <div class="glass-card requirements-card" v-if="false">
