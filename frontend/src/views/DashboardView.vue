...
const totalBalance = computed(() => {
    ...
    const total = parseFloat(wallet.value.withdrawable_balance || 0)
...
const isAlreadyDeposited = computed(() => {
    ...
    const currentBalance = parseFloat(userStore.wallet.withdrawable_balance || 0)
...
onMounted(async () => {
    loadingStore.startLoading(true) // Full screen for initial load
    try {
...
<template>
  <div class="dashboard-wrapper">
    <!-- Screen Loader removed, handled globally by App.vue -->
    ...
    <div class="content-scroll">
      ...
      <!-- User Info Row -->
      <div class="user-info w-full flex items-center gap-2 mb-1">
      ...
      <div class="earning-card">
        ...
        <div class="relative z-10 w-full">
           ...
           <h1 class="text-3xl font-black text-white tracking-tighter mb-3">
             <span class="text-yellow-500 text-xl align-top">₹</span>{{ totalBalance }}
           </h1>
      ...
      <div class="relative overflow-hidden rounded-[2rem] p-[1px] group cursor-pointer" @click="$router.push('/question')">
        ...
        <div class="relative bg-[#111] rounded-[2rem] p-4 flex items-center justify-between h-full overflow-hidden border border-white/5">
            ...
            <div class="relative z-10 flex-1">
              <div class="flex items-center gap-2 mb-1">
                 <span class="bg-gradient-to-r from-yellow-500/20 to-orange-500/20 text-yellow-400 text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-wider border border-yellow-500/20">Daily Quest</span>
                 <span class="animate-pulse w-1.5 h-1.5 rounded-full bg-green-500"></span>
              </div>
              <h3 class="font-black text-white text-xl tracking-tighter italic bg-gradient-to-r from-white to-gray-400 bg-clip-text text-transparent">QUIZ BATTLE</h3>
              <p class="text-[9px] text-gray-400 mt-0.5 font-medium max-w-[130px] leading-relaxed">
                 Challenge yourself & <span class="text-yellow-500 font-bold">Earn Rewards!</span>
              </p>
            </div>

            <div class="relative z-10 pl-3">
               <button class="relative w-14 h-14 rounded-full bg-gradient-to-br from-yellow-400 via-orange-500 to-red-500 flex items-center justify-center shadow-lg shadow-orange-500/30 group-active:scale-95 transition-all duration-300 border-4 border-[#111]">
                  <div class="absolute inset-0 bg-white/20 rounded-full animate-ping opacity-20"></div>
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-black fill-current translate-x-0.5 drop-shadow-sm" viewBox="0 0 24 24">
...
.avatar-circle {
    @apply w-10 h-10 rounded-full p-[2px] bg-gradient-to-tr from-yellow-400 to-yellow-600 shadow-lg shadow-yellow-500/20;
}
.avatar-circle img {
...
/* Content */
.content-scroll {
    @apply px-4 pt-3 pb-24 flex flex-col gap-4;
}
