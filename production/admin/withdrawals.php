...
$sql = "SELECT w.*, u.name, u.email, u.bank_account_number, u.usdt_address FROM withdraws w JOIN users u ON w.user_id = u.id $where ORDER BY w.created_at DESC LIMIT $limit OFFSET $offset";
                ...
                <tbody class="divide-y divide-white/5 text-sm">
                    ...
                    <tr class="hover:bg-white/5 transition-colors">
                        ...
                        <td class="p-4 text-xs font-bold text-gray-400">
                         ...
                         <td class="p-4 text-[10px] text-gray-400 max-w-xs break-all">
                             <?php if ($w['usdt_address']): ?>
                                 Addr: <?= htmlspecialchars($w['usdt_address']) ?>
                             <?php else: ?>
                                 Acc: <?= htmlspecialchars($w['bank_account_number']) ?>
                             <?php endif; ?>
                        </td>
                        <td class="p-4">
